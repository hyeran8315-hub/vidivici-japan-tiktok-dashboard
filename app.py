...
DATA_PATTERN = "vidivici_comments_*.csv"
...
CATEGORY_REFINEMENT = {
    "호감·감탄": [
        "最高", "すごい", "素敵", "好き", "かわいい", "可愛い",
        "美しい", "綺麗", "きれい", "神", "尊い", "良い", "いい"
    ],
    "제품 관심·탐색": [
...
def load_data():
    ...
    # 누적 운영:
    # - 기본 파일 vidivici_comments_final.csv
    # - 추가로 vidivici_comments_*.csv 파일이 있으면 함께 병합
    # - 동일 댓글은 자동 중복 제거
    candidate_files = sorted(base_dir.glob(DATA_PATTERN))

    if Path(DATA_FILE).exists() and Path(DATA_FILE) not in candidate_files:
        candidate_files.insert(0, Path(DATA_FILE))

    if not candidate_files:
        st.error("댓글 CSV 파일을 찾을 수 없습니다.")
        st.stop()
    ...
    for file_path in candidate_files:
        try:
            ...
            tmp["_source_file"] = file_path.name
            frames.append(tmp)
        except Exception:
            continue

    if not frames:
        st.error("읽을 수 있는 댓글 CSV 파일이 없습니다.")
        st.stop()
    ...
    if "uniqueId" in df.columns:
        ...
        dedupe_cols = [c for c in ["text", "videoWebUrl"] if c in df.columns]
        if dedupe_cols:
...
st.markdown(
    '<div class="sub-title">일본 TikTok 댓글을 누적해 콘텐츠 반응과 제품 인사이트를 확인합니다.</div>',
    unsafe_allow_html=True,
)
...
if start_date is not None and end_date is not None:
    st.caption(
        ...
        f"누적 파일 {df['_source_file'].nunique() if '_source_file' in df.columns else 1}개"
    )
...
if not valid_dates.empty:
    ...
    period_mode = st.sidebar.radio(
        "기간 선택",
        ["전체", "최근 7일", "최근 30일", "직접 선택"],
        horizontal=False,
        label_visibility="collapsed",
    )

    if period_mode == "최근 7일":
...
st.sidebar.markdown("---")
if HAS_TRANSLATION:
    translation_enabled = st.sidebar.toggle(
        "한국어 번역 표시",
        value=True,
...
if start_date is not None and end_date is not None:
    date_mask = (
        filtered_df["createTimeISO"].dt.date >= pd.to_datetime(start_date).date()
    ) & (
        filtered_df["createTimeISO"].dt.date <= pd.to_datetime(end_date).date()
    )
...
filtered_df = filtered_df[
    ...
    & filtered_df["reaction_target"].isin(selected_reactions)
].copy()
...
# -----------------------------
st.subheader("댓글 추이")
st.caption("현재 필터 기준 일별 댓글 수")

trend_source = filtered_df.dropna(subset=["createTimeISO"]).copy()

if not trend_source.empty:
    trend_source["date"] = trend_source["createTimeISO"].dt.date.astype(str)
    trend_df = (
        trend_source.groupby("date")
        .size()
        .reset_index(name="댓글 수")
    )
    ...
    trend_chart = (
        ...
        .encode(
            ...
            y=alt.Y(
                "댓글 수:Q",
                title="댓글 수",
                axis=alt.Axis(grid=True, gridColor=LIGHT_GRAY, domain=False),
            ),
            tooltip=["date:T", "댓글 수:Q"],
        )
    ...
    st.info("댓글 작성일 데이터가 없어 기간별 추이를 표시할 수 없습니다.")
...
if translation_enabled and "translation_ko" in filtered_df.columns:
...
st.caption(
    ...
    "업로드된 누적 CSV를 병합하고 중복을 제거해 집계합니다."
)
